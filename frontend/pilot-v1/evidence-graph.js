(()=>{
    ...
    panel.innerHTML=`
      <div class="eg-toolbar">
        <div>
          ...
          <div class="note">A recuperação documental apenas <strong>informa</strong>. Suporte, contradição, validação e decisão exigem curadoria explícita.</div>
        </div>
  ...
  function provenanceText(node){
    ...
    if(node.source_kind==='document_chunk'){
      ...
      return `${esc(filename)} · posição ${node.char_start??'—'}–${node.char_end??'—'}${hash?` · hash ${esc(hash)}…`:''}${ranks?` · ${esc(ranks)}`:''}`;
    }
  ...
  function render(graph){
    ...
    if(nodeRoot){
      nodeRoot.innerHTML=nodes.length?nodes.map(node=>{
        ...
        return `<article class="eg-node" data-type="${esc(node.node_type)}"><div class="eg-node-head"><div><span class="eg-type">${esc(typeLabels[node.node_type]||node.node_type)}</span><strong style="display:block;margin-top:8px">${esc(node.label)}</strong></div><span class="pill">${esc(statusLabels[node.status]||node.status)}</span></div><p>${esc((node.body||'').slice(0,2200))}</p><div class="eg-prov">${provenanceText(node)} · <span class="eg-confidence">confiança ${confidence}</span></div>${outgoing?`<div class="eg-edges">${outgoing}</div>`:''}</article>`;
