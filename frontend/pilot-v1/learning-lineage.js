(()=>{
  ...
  function render(data){
    ...
    list.innerHTML=(data.candidates||[]).length?data.candidates.map(candidate=>{
      ...
      const humanReview=!review.system_enforced;
      const applicability=review.applicability||'';
      const suspended=candidate.canonical_status==='suspended';
      const cssClass=suspended?'ll-suspended':applicability==='reuse'?'ll-active':applicability==='requires_revalidation'?'ll-revalidate':applicability==='not_applicable'?'ll-not-applicable':'';
      ...
      const canonicalLabel={valid:'canonicamente válida',suspended:'validade suspensa',superseded:'substituída',archived:'arquivada',invalidated:'canonicamente invalidada'}[candidate.canonical_status]||candidate.canonical_status||'estado canónico desconhecido';
      const actions=suspended
        ? '<button class="btn btn-primary" type="button" data-applicability="requires_revalidation">Revalidar antes de reutilizar</button><button class="btn btn-ghost" type="button" data-applicability="not_applicable">Não aplicável a esta missão</button>'
        : '<button class="btn btn-ghost" type="button" data-applicability="reuse">Reutilizar nesta missão</button><button class="btn btn-ghost" type="button" data-applicability="requires_revalidation">Revalidar antes de reutilizar</button><button class="btn btn-ghost" type="button" data-applicability="not_applicable">Não aplicável a esta missão</button>';
