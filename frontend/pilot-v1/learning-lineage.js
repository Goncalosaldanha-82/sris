(()=>{
  ...
  function install(){
    ...
    style.textContent=`
      ...
      .ll-list{display:grid;gap:11px}.ll-card{border:1px solid var(--line);border-radius:15px;padding:15px;background:#fff}.ll-top{display:flex;justify-content:space-between;gap:12px}.ll-source{font-size:10px;color:var(--muted);margin:5px 0}.ll-statement{line-height:1.6;white-space:pre-wrap}.ll-lineage{display:flex;gap:6px;flex-wrap:wrap;margin:10px 0}.ll-chip{font-size:9px;padding:4px 7px;border-radius:999px;background:#eef4f1;color:#48695e}.ll-actions{display:flex;gap:7px;flex-wrap:wrap;border-top:1px solid var(--line);padding-top:10px;margin-top:10px}.ll-actions button{padding:8px 10px}.ll-active{border-left:4px solid #2f765f}.ll-revalidate{border-left:4px solid #d49b3e}.ll-invalid{opacity:.72}
  ...
  function render(data){
    ...
    if(summaryRoot)summaryRoot.innerHTML=[
      ...
      ['still_valid_count','Ainda válidas'],
      ['requires_revalidation_count','Revalidar'],
      ['invalidated_count','Invalidadas'],
    ...
    list.innerHTML=(data.candidates||[]).length?data.candidates.map(candidate=>{
      ...
      const cssClass=review.disposition==='still_valid'?'ll-active':review.disposition==='requires_revalidation'?'ll-revalidate':review.disposition==='invalidated'?'ll-invalid':'';
      const counts=candidate.lineage?.counts||{};
      return `<article class="ll-card ${cssClass}" data-packet="${esc(candidate.id)}">
        <div class="ll-top"><div><strong>${esc(candidate.title)}</strong><div class="ll-source">${esc(candidate.source_mission?.code)} · ${esc(candidate.source_mission?.title||'')} · relevância ${Math.round((candidate.relevance_score||0)*100)}%</div></div><span class="pill">${esc(review.disposition||'por rever')}</span></div>
        ...
        <div class="ll-actions"><button class="btn btn-ghost" type="button" data-disposition="still_valid">Ainda válida</button><button class="btn btn-ghost" type="button" data-disposition="requires_revalidation">Requer revalidação</button><button class="btn btn-ghost" type="button" data-disposition="invalidated">Invalidada</button></div>
        <form class="ll-review-form hidden" data-review-form data-disposition="">
          <h4 data-review-title>Rever aprendizagem</h4>
          <div class="field"><label>Justificação da revisão *</label><textarea data-review-rationale required maxlength="5000" placeholder="Explique porque esta aprendizagem continua válida, precisa de revalidação ou deixou de se aplicar.">${esc(review.rationale||'')}</textarea></div>
  ...
  function handleClick(event){
    ...
    form.dataset.disposition=disposition;
    const labels={still_valid:'Confirmar que continua válida',requires_revalidation:'Marcar para revalidação',invalidated:'Invalidar neste contexto'};
    const title=form.querySelector('[data-review-title]');
    if(title)title.textContent=labels[disposition]||'Rever aprendizagem';
    const contextField=form.querySelector('[data-context-field]');
    const context=form.querySelector('[data-review-context]');
    const requiresChange=disposition!=='still_valid';
  ...
  async function handleReviewSubmit(event){
    ...
    const disposition=form.dataset.disposition;
    const code=missionCode();
    ...
    if(!packetId||!code||!disposition)return;
    if(!rationale){if(message){message.textContent='Explique a decisão de revisão.';message.classList.add('error');}return;}
    if(disposition!=='still_valid'&&!contextChange){if(message){message.textContent='Registe o que mudou no contexto.';message.classList.add('error');}return;}
    ...
    try{
      await api(`/api/pilot/learning/missions/${encodeURIComponent(code)}/candidates/${encodeURIComponent(packetId)}/review`,{
        method:'POST',
        body:JSON.stringify({disposition,rationale,context_change:disposition==='still_valid'?'':contextChange}),
      });
      await load();
      document.dispatchEvent(new CustomEvent('sris:learning-reviewed',{detail:{mission_code:code,packet_id:packetId,disposition}}));
    }catch(error){
