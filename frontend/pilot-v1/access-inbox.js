(()=>{
  ...
  function licenceTerms(row){
    const e=row.entitlement;
    if(!e?.present)return row.status==='approved'?'<p class="message error">Sem licença comercial atribuída.</p>':'';
    return`<div class="notice"><strong>Prazo comercial do workspace</strong><p>Início: ${date(e.starts_at)}.<br>${e.expires_at?`Válido até: <strong>${date(e.expires_at)}</strong>.`:'<strong>Sem prazo definido: autorização antiga excecional.</strong>'}<br>Estado: ${esc({active:'Ativo',expired:'Expirado',suspended:'Suspenso',cancelled:'Cancelado'}[e.status]||e.status)}.</p><p>Sem renovação automática e sem cobrança automática. Ativar a conta ou reenviar o convite não prolonga este prazo. O plano, por si só, não comprova um pagamento.</p></div>`;
  }
