(()=>{
  ...
  $('#requests').addEventListener('click',event=>{
    ...
    if(button.hasAttribute('data-approve')){
      ...
      if(!confirm(`Aprovar ${row.full_name} (${row.email})?\nNovo workspace: ${payload.workspace_name}\nPlano: ${payload.plan_code} · ${payload.entitlement_days} dias\nSerá enviado um convite pessoal.`))return;
      action(button,async()=>{await request(base+'/approve',{method:'POST',body:JSON.stringify(payload)});message('Pedido aprovado. O envio do convite pode ser confirmado no histórico de aprovados.');});
    ...
    else if(button.hasAttribute('data-resend')){if(!confirm('Reenviar o convite para '+row.email+'? O endereço anterior deixará de ser válido.'))return;action(button,async()=>{await request(base+'/resend-invitation',{method:'POST'});message('Novo convite preparado para envio.');});}
