...
(()=>{
  ...
  const POLICY_SOURCES={platform_default:'Política predefinida da plataforma',organization_policy:'Política da organização',mission_override:'Exceção revista para a missão'};
  let currentCode='';
  ...
  function render(){
    ...
    const critical=Number(state.health?.critical_conflicts||0);
    const criticalLabel=`${critical} conflito${critical===1?' crítico':'s críticos'}`;
    const strip=document.querySelector('#governed-mission-context');
    if(strip)strip.innerHTML=`<strong>${esc(state.mission.code)} · estado governado único</strong><span>${esc(health[0])} · ${criticalLabel}</span>`;
    root.innerHTML=`
      <div class="gms-head">
      ...
      <details class="gms-integrity"><summary>Integridade verificada · supervisão humana obrigatória</summary><div class="gms-integrity-proof"><span>${esc(POLICY_SOURCES[policy.source]||'Política técnica registada')}</span><code>SHA-256 ${esc(state.state_hash||'a sincronizar')}</code></div></details>`;
  }
