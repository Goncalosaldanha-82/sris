(function () {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .bc-quality-dimensions{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;margin:12px 0}.bc-quality-dimensions>div{display:grid;gap:4px;border:1px solid #d9e1de;border-radius:10px;background:#f8faf9;padding:9px}.bc-quality-dimensions span{color:#62766e;font-size:9px;text-transform:uppercase;letter-spacing:.04em}.bc-quality-dimensions strong{font-size:11px}
  ...
  function scenarioTable() {
    ...
    const rows = ["conservative", "base", "favorable"].map((key) => {
      ...
      return `<tr><th scope="row">${labels[key]}</th><td>${metricMoney(row.total_cost,`scenario_${key}_costs`)}</td><td>${metricMoney(row.gross_benefit,`scenario_${key}_benefits`)}</td><td>${metricMoney(row.net_benefit,financialGroup)}</td><td>${!metricKnown(financialGroup)||row.roi_pct == null?"—":`${metricNumber(row.roi_pct,2,financialGroup)}%`}</td><td>${!metricKnown(financialGroup)||row.payback_months == null?"—":`${metricNumber(row.payback_months,0,financialGroup)} meses`}</td><td>${metricMoney(row.npv,financialGroup)}</td></tr>`;
    }).join("");
  ...
  function render() {
    ...
    const readiness = state.readiness || {};
    const structuralProgress = Number(readiness.progress_percent || 0);
    const reviewState = current.status === "reviewed" ? "Revisto por uma pessoa" : "Por rever";
    ...
    const history = (state.history || []).length
      ? state.history.map((item) => `<div class="bc-history-row"><span><strong>Revisão ${item.revision}</strong> · ${esc(EVENT_LABELS[item.event_type] || item.event_type)}</span><span class="bc-muted">${item.created_at ? new Date(item.created_at).toLocaleString("pt-PT") : ""} · ${item.content_hash ? "integridade verificada" : "integridade por confirmar"}</span></div>`).join("")
      : '<div class="bc-muted">Ainda não existem revisões económicas.</div>';
    const historyProof=(state.history||[]).some(item=>item.content_hash)?`<details class="technical-integrity"><summary>Prova técnica das revisões</summary><div class="technical-proof">${state.history.filter(item=>item.content_hash).map(item=>`<code>Revisão ${Number(item.revision||1)} · SHA-256 ${esc(item.content_hash)}</code>`).join('')}</div></details>`:'';
    ...
    const qualitySummary = qualityKnown
      ? `<p>Confiança declarada ${number(quality.confidence_score, 0)}% · origem ${number(quality.source_coverage_pct, 0)}% · evidência ${number(quality.evidence_coverage_pct, 0)}%.</p><div class="bc-quality-dimensions"><div><span>Rastreabilidade das linhas monetárias</span><strong>${number(qualityScore, 0)}%</strong></div><div><span>Completude estrutural</span><strong>${Number(readiness.completed_checks || 0)}/${Number(readiness.total_checks || 0)} · ${number(structuralProgress, 0)}%</strong></div><div><span>Estado de revisão</span><strong>${reviewState}</strong></div></div>`
    ...
    const configuredSections = current.id ? [
      ...
      `<section class="bc-card"><div class="bc-quality"><div class="bc-quality-score" style="--score:${qualityScore}"><strong>${qualityKnown ? `${number(qualityScore, 0)}%` : "—"}</strong><span>rastreabilidade</span></div><div><h4>Qualidade e prontidão dos dados económicos</h4>${qualitySummary}<p class="bc-muted">A pontuação mede completude e rastreabilidade; não transforma estimativas em factos. A rastreabilidade, a completude estrutural e a revisão humana são dimensões independentes.</p></div></div></section>`,
      reviewCard(),
      `<details class="bc-card"><summary><strong>Histórico económico preservado</strong></summary><div class="bc-history" style="margin-top:14px">${history}</div>${historyProof}</details>`,
    ].filter(Boolean).join("") : "";
    node.innerHTML = `
