(function () {
  ...
  function scenarioTable() {
    ...
    const rows = ["conservative", "base", "favorable"].map((key) => {
      ...
      return `<tr><th scope="row">${labels[key]}</th><td>${money(row.total_cost)}</td><td>${money(row.gross_benefit)}</td><td>${money(row.net_benefit)}</td><td>${row.roi_pct == null ? "—" : `${number(row.roi_pct, 2)}%`}</td><td>${row.payback_months == null ? "—" : `${number(row.payback_months, 0)} meses`}</td><td>${money(row.npv)}</td></tr>`;
    }).join("");
  ...
  function alternativeTable() {
    ...
    const rows = profiles.map((profile) => {
      ...
      const resourceText = [
        resources.planned_human_hours ? `${number(resources.planned_human_hours)} h` : "",
      ...
      return `<tr><th scope="row"><strong>${esc(profile.alternative_label)}</strong>${gaps}</th><td>${money(profile.total_cost)}</td><td>${esc(resourceText)}</td><td>${money(profile.probable_gross_benefit)}</td><td>${money(profile.probable_net_benefit)}</td><td>${profile.roi_pct == null ? "—" : `${number(profile.roi_pct, 2)}%`}</td><td>${profile.payback_months == null ? "—" : `${number(profile.payback_months, 0)} meses`}</td><td>${number(profile.quality?.overall_score || 0, 0)}%</td></tr>`;
    }).join("");
  ...
  function caseForm() {
    ...
    return `<form id="bc-case-form">
      <div class="bc-grid">
        ...
        <div class="bc-field"><label for="bc-horizon">Horizonte de análise</label><input id="bc-horizon" name="horizon_months" type="number" min="1" max="600" step="1" value="${esc(item.horizon_months || 60)}" required><small>meses, incluindo efeitos posteriores</small></div>
        <div class="bc-field"><label for="bc-discount">Taxa anual de desconto</label><input id="bc-discount" name="discount_rate_pct" type="number" min="0" max="100" step="0.01" value="${esc(inputValue(item.discount_rate_pct ?? 8))}" required><small>% usada no VAL</small></div>
  ...
  function itemForm() {
    ...
    return `<details class="bc-form-shell" ${existing || !(state.items || []).length ? "open" : ""}><summary>${esc(title)}</summary><div class="bc-form-body"><form id="bc-item-form" data-item-id="${esc(existing?.id || "")}">
      <div class="bc-grid">
        ...
        <div class="bc-field"><label for="bc-treatment">Tratamento no cálculo</label><select id="bc-treatment" name="financial_treatment"><option value="cost" ${item.financial_treatment === "cost" ? "selected" : ""}>Somar aos custos</option><option value="benefit" ${item.financial_treatment === "benefit" ? "selected" : ""}>Somar aos benefícios</option><option value="none" ${item.financial_treatment === "none" ? "selected" : ""}>Quantificar sem monetizar</option></select></div>
      ...
      <div class="bc-grid" style="margin-top:12px">
        ...
        <div class="bc-field"><label for="bc-unit">Unidade</label><input id="bc-unit" name="unit" maxlength="80" value="${esc(item.unit || "")}" placeholder="Ex.: horas, unidades, kg"></div>
  ...
  function lineRows() {
    ...
    return `<div class="bc-ledger">${items.map((item) => {
      ...
      return `<article class="bc-line"><div class="bc-line-main"><span class="bc-kind">${esc(KIND_LABELS[item.kind] || item.kind)}</span>${scope}<strong>${esc(item.label)}</strong><small>${esc(PHASE_LABELS[item.phase] || item.phase)} · ${esc(RECURRENCE_LABELS[item.recurrence] || item.recurrence)} · ${esc(status)} · confiança ${esc(item.confidence)}</small><small>${esc(item.source_label || item.evidence_label || "origem não declarada")}</small>${blocked}</div><div class="bc-line-value"><strong>${money(item.base_amount)}</strong><span>${item.amount_basis === "per_unit" ? "base / unidade" : "base / ocorrência"}</span></div><div class="bc-line-value"><strong>${money(item.realized_amount)}</strong><span>realizado acumulado</span></div><div class="bc-line-value"><strong>${item.kind === "human_resource" ? `${number(item.actual_quantity || 0)} ${esc(item.unit || "")}` : money(item.forecast_amount)}</strong><span>${item.kind === "human_resource" ? "esforço observado" : "projeção"}</span></div>${retirement}</article>`;
    }).join("")}</div>`;
  }
  ...
  document.addEventListener("change", (event) => {
    if (event.target.id === "bc-item-kind") {
      ...
      if (treatment && forced) treatment.value = forced;
    }
