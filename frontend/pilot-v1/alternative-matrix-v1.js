(function () {
  ...
  function render() {
    ...
    const history = (state.history || []).length
      ? state.history.map((item) => `<div class="alternative-matrix-history-row"><span><strong>Revisão ${item.revision}</strong> · ${esc(item.status === "reviewed" ? "revista" : "rascunho")} · ${item.alternative_count} alternativas</span><span class="note">${item.integrity_verified ? "Integridade verificada" : "Integridade inválida"}</span></div>`).join("")
    ...
    node.innerHTML = `
      ...
      <details class="alternative-matrix-card"><summary><strong>Histórico imutável e integridade</strong></summary><div class="alternative-matrix-history" style="margin-top:14px">${history}</div>${historyProof}</details>
    `;
    bind();
  ...
  async function save() {
    try {
      ...
      render();
      setStatus(`Revisão ${state.matrix.revision} guardada com integridade verificada.`, "success");
